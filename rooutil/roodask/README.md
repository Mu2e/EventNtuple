...
```json
{
  ...
  "compile_flags": [],
  "output_dir": "./output",
  "output_pattern": "output_{job_id}.root",
...
2. Splits input files into batches of `--files-per-job` files
3. **Compiles** the C++ macro with `g++` and `root-config` flags
   (unless `--skip-compile`)
4. Starts a `LocalCluster` or connects to an existing scheduler
5. For each batch/job, submits a task to a Dask worker that:
...
6. Collects results as jobs complete and prints progress
7. Writes `results.json` with all job outcomes
...
```json
{
  ...
  "output_file": "./output/output_job_0000.root"
}
```
